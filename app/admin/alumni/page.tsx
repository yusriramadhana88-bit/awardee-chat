...
export default function AdminAlumniPage() {
  ...
  if (loading) return <div className="p-8 text-muted text-sm">Memuat data alumni...</div>
  ...
  return (
    <div className="px-6 py-8 max-w-6xl mx-auto space-y-8">
      <div>
        <h1 className="text-xl font-bold text-ink mb-2">Awardee Alumni ({alumni.length})</h1>
      ...
      {pending.length > 0 && (
        <section>
          <h2 className="text-sm font-semibold text-ink mb-3">Menunggu Review ({pending.length})</h2>
            ...
            {pending.map(a => (
              <div key={a.id} className="p-4 flex flex-col sm:flex-row sm:items-center gap-3">
                <div className="flex-1 min-w-0">
                  <div className="font-medium text-ink">{a.profiles?.name ?? 'Unknown'} <span className="text-xs text-muted font-normal">{a.profiles?.email}</span></div>
                  <div className="text-sm text-muted mt-0.5">{a.scholarship_name}{a.university ? ` · ${a.university}` : ''}{a.graduation_year ? ` · ${a.graduation_year}` : ''}</div>
                  {a.story && <div className="text-xs text-muted mt-1 line-clamp-2">{a.story}</div>}
                </div>
                <div className="flex gap-2 shrink-0">
                  <button onClick={() => handleAction(a.id, 'approve')} disabled={updating === a.id}
                    className="bg-green-600 hover:bg-green-700 disabled:bg-hairline text-white text-xs font-semibold px-3 py-2 rounded-lg transition-colors">
                    Setujui
                  </button>
                  <button onClick={() => handleAction(a.id, 'reject')} disabled={updating === a.id}
                    className="bg-off hover:bg-hairline text-muted text-xs font-semibold px-3 py-2 rounded-lg transition-colors">
                    Tolak
                  </button>
                </div>
              </div>
      ...
      <section>
        <h2 className="text-sm font-semibold text-ink mb-3">Alumni Aktif ({approved.length})</h2>
        {approved.length === 0 ? (
          <div className="bg-white rounded-xl border border-hairline p-8 text-center text-muted text-sm">Belum ada alumni aktif.</div>
        ) : (
                ...
                <tbody>
                  {approved.map(a => (
                    <tr key={a.id} className="border-b border-hairline">
                      ...
                      <td className="px-4 py-3 text-muted">{a.scholarship_name}</td>
                      <td className="px-4 py-3 text-center font-semibold text-gold-2">{a.contribution_count}</td>
                      <td className="px-4 py-3 text-center text-muted">{a.contribution_hours}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {rejected.length > 0 && (
        <section>
          <h2 className="text-sm font-semibold text-ink mb-3">Ditolak ({rejected.length})</h2>
          <div className="bg-white rounded-xl border border-hairline divide-y divide-hairline">
            {rejected.map(a => (
              <div key={a.id} className="p-4 text-sm text-muted">{a.profiles?.name ?? 'Unknown'} · {a.scholarship_name}</div>
            ))}
          </div>
        </section>
      )}
