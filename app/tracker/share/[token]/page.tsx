      ...
      <main className="max-w-2xl mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-6 mb-5 shadow-sm">
          ...
          <div className="flex items-center justify-between text-sm">
            <span className="text-gray-500">{completedStages}/{data.stages.length} tahapan selesai</span>
            <div className="flex items-center gap-2">
              {data.deadline && (
                <span className="text-gray-400 text-xs">{formatDeadline(data.deadline)}</span>
              )}
              {deadline && (
        ...
        {/* Stages */}
        <h2 className="text-sm font-semibold text-gray-700 mb-3">Tahapan Aplikasi</h2>
        <div className="space-y-3">
          {data.stages.map((stage, idx) => {
            ...
            return (
              <div
                key={stage.id}
                className={`bg-white rounded-xl border p-4 ${isDone ? 'border-green-200 bg-green-50/30' : 'border-gray-200'}`}
              >
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className={`font-medium text-sm flex-1 ${isDone ? 'text-gray-400 line-through' : 'text-gray-800'}`}>
                  ...
                  <div className="ml-9 space-y-1.5">
                    {stage.checklist_items.map(item => (
                      <div key={item.id} className="flex items-center gap-2">
                        ...
                        <span className={`text-xs ${item.is_completed ? 'line-through text-gray-400' : 'text-gray-600'}`}>
                          {item.text}
                        </span>
                      </div>
                    ))}
                ...
                {stage.notes && (
                  <div className="ml-9 mt-2 text-xs text-gray-500 bg-gray-50 rounded-lg px-3 py-2">
                    {stage.notes}
                  </div>
                )}
              </div>
