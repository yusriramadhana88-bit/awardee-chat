...
export default function AffiliatePage() {
  ...
  return (
    <div className="px-4 py-6 lg:px-8 lg:py-8 max-w-4xl mx-auto">
      <div className="mb-6">
        <h1 className="text-xl font-bold text-gray-900">Afiliasi & Komisi</h1>
        <p className="text-sm text-gray-500 mt-0.5">Ajak teman daftar AWARDEE APP, dapatkan komisi 20% dari setiap subscription yang berhasil.</p>
      </div>

      {!affiliate ? (
        <div className="bg-white rounded-xl border border-gray-200 p-8 text-center">
          <div className="text-4xl mb-3">🤝</div>
          <h2 className="font-semibold text-gray-900 text-lg mb-2">Bergabung Program Afiliasi</h2>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              <div className="text-2xl font-bold text-gray-800">{stats?.clicks ?? 0}</div>
              <div className="text-xs text-gray-400 mt-1">Total klik link</div>
            </div>
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              <div className="text-2xl font-bold text-green-600">{stats?.conversions ?? 0}</div>
              <div className="text-xs text-gray-400 mt-1">Konversi berhasil</div>
            </div>
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              <div className="text-xl font-bold text-sky-600">Rp {Number(affiliate.total_earned).toLocaleString('id-ID')}</div>
              <div className="text-xs text-gray-400 mt-1">Total komisi earned</div>
            </div>
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              <div className="text-xl font-bold text-purple-600">Rp {Number(stats?.pendingCommission ?? 0).toLocaleString('id-ID')}</div>
              <div className="text-xs text-gray-400 mt-1">Menunggu pembayaran</div>
            </div>
          </div>

          {/* Referral link */}
          <div className="bg-sky-50 border border-sky-200 rounded-xl p-4">
            <div className="text-sm font-medium text-gray-700 mb-2">Link Referral Kamu</div>
            <div className="flex items-center gap-2">
              <code className="flex-1 text-sm bg-white border border-sky-200 rounded-lg px-3 py-2.5 text-sky-700 font-mono truncate">
                {referralLink}
              </code>
              <button
                onClick={copyLink}
                className="bg-sky-600 hover:bg-sky-700 text-white text-sm px-4 py-2.5 rounded-lg font-medium whitespace-nowrap transition-colors"
              >
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            <h2 className="font-semibold text-gray-900 text-sm mb-3">Riwayat Referral</h2>
            {referrals.length === 0 ? (
              <p className="text-sm text-gray-400">Belum ada referral. Share link kamu dan mulai dapatkan komisi!</p>
            ) : (
                  ...
                  <tbody>
                    {referrals.map(r => (
                      <tr key={r.id} className="border-b border-gray-50">
                        <td className="py-2 pr-4 text-gray-600">{new Date(r.clicked_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}</td>
                        <td className="py-2 pr-4 text-gray-600">{r.converted_at ? new Date(r.converted_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' }) : '—'}</td>
                        ...
                        <td className="py-2">
                          <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full ${
                            ...
                            r.payout_status === 'approved' ? 'bg-sky-100 text-sky-700' :
